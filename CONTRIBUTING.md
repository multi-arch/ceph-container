...
Project structure
-----------------
The primary deliverables of this project are three container images: `client`, `daemon-base`, and `daemon`. As
...
in `src/` and is further specified in `src/client`, `src/daemon-base` and `src/daemon`.
...
`<staging>/client/`, `<staging>/daemon-base/` and `<staging>/daemon/` image directories. Additionally, developers can
...
ceph-releases/<ceph release>/<base os repository>/{client,daemon-base,daemon}/FILE
ceph-releases/<ceph release>/<base os repository>/FILE
ceph-releases/<ceph release>/{client,daemon-base,daemon}/FILE
ceph-releases/<ceph release>/FILE
ceph-releases/ALL/<base os repository>/<base os release>/{client,daemon-base,daemon}/FILE
ceph-releases/ALL/<base os repository>/<base os release>/FILE
ceph-releases/ALL/<base os repository>/{client,daemon-base,daemon}/FILE
ceph-releases/ALL/<base os repository>/FILE
ceph-releases/ALL/{client,daemon-base,daemon}/FILE
ceph-releases/ALL/FILE
src/{client,daemon-base,daemon}/FILE
src/FILE
# Least specific
```
...
the desired build command for `client`, `daemon-base` and `daemon`.
```
# Example
cd <staging>
docker build -t <client tag> client/
...
2. Add a `ceph-releases/ALL/<new distro>` directory for the new distro
   - Make sure to install all the required packages for `client` (see 
     `src/client/__CEPH_BASE_PACKAGES__)`, `daemon-base` (see
